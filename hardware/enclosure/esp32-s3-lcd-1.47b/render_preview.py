...
for name,material in [('front-bezel',shellmat),('clearance-tray',wingmat)]:
 bpy.ops.wm.stl_import(filepath=str(out/(name+'.stl')));o=bpy.context.object;o.name=name;o.data.materials.append(material)
 if name=='front-bezel':o.rotation_euler.x=math.pi;o.location.z=14.5
bpy.ops.mesh.primitive_cube_add(size=1,location=(0,0,12.55));o=bpy.context.object;o.name='Illustrative screen';o.dimensions=(20.8,36.8,.9);bpy.ops.object.transform_apply(location=False,rotation=False,scale=True);o.data.materials.append(glass);bevel=o.modifiers.new('Rounded glass','BEVEL');bevel.width=2;bevel.segments=6
...
for x in [-5,5]:
 bpy.ops.mesh.primitive_uv_sphere_add(segments=24,ring_count=12,location=(x,4,13.05))
...
for i,point in enumerate(spline.points):
 t=math.pi+math.pi*i/32;point.co=(4*math.cos(t),-3+3*math.sin(t),13.05,1)
...
# Exploded view exposes the recessed tray lip and snap catches.
bpy.data.objects['front-bezel'].location.z+=18
for o in bpy.data.objects:
 if o.name in ['Illustrative screen','Smile'] or o.name.startswith('Eye'):o.hide_render=True
cam.data.ortho_scale=115
s.render.filepath=str(out/'exploded.png');bpy.ops.render.render(write_still=True)

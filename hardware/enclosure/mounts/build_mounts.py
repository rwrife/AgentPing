...
PAD_EXTRA = 4.  # Extend the adhesive face toward the monitor, away from the arm.
STANDOFF = 4.
...
def arm(desk=False):
    ...
    bottom=15-LENGTH
    if not desk:
        s=s.fuse(round_plate(MONITOR_PAD,MONITOR_PAD,3,-10-PAD_EXTRA,THICKNESS+PAD_EXTRA,cy=bottom+MONITOR_PAD/2))
...
# remaining arm 4 mm off a planar monitor at the pad's rear face.
monitor = parts['monitor-arm']
pad_bottom = 15-LENGTH
added = round_plate(MONITOR_PAD,MONITOR_PAD,3,-10-STANDOFF-PAD_EXTRA,PAD_EXTRA,cy=pad_bottom+MONITOR_PAD/2)
assert added.cut(monitor).Volume < .001
clearance = box(100,LENGTH-MONITOR_PAD,PAD_EXTRA,-50,pad_bottom+MONITOR_PAD,-10-STANDOFF-PAD_EXTRA)

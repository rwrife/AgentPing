...
Run `python3 protocol/validate.py` to validate the schema, fixtures, 16 KiB wire bound, and generated firmware constants. Run `python3 protocol/validate.py --write-header` only after deliberately changing schema limits. Regenerate the human-readable reference with `python3 protocol/generate_reference.py` and enforce freshness in CI with `python3 protocol/generate_reference.py --check`. The bridge serialization tests consume the same fixtures.

...
The protocol-v1 schema and pairing design are checked in and statically tested. The bridge implements loopback provider ingestion, atomic state persistence, digest-authenticated WebSockets, capability/heartbeat validation, bounded reconnect replay, and live state fan-out. Firmware implements an RFC1918-only WSS client, exact leaf-certificate trust anchor, serial credential provisioning, persistent resume checkpoints, and fail-closed message reduction; this has compile/host-test evidence only. The stack still has no pairing endpoint/UI, Windows protected token issuance/rotation, LAN TLS certificate provisioning, physical device validation, or approval executor. See [`docs/protocol.md`](docs/protocol.md) for the threat model and requirements. Keep the default listener on loopback until those remaining controls are implemented.

...
try{
 ...
 for(const [name,state,count,seconds,agent] of [['idle','idle',24,1.958333,'codex'],['wave','waiting',36,4.76,'codex'],['wave-claude','waiting',36,4.76,'claude'],['wave-copilot','waiting',36,4.76,'copilot']]){
  ...
  for(let i=0;i<count;i++){
   const data=await page.evaluate(({t,name})=>{
    ...
    const halfHeight=name.startsWith('wave')?1.3:1.05;c.left=-halfHeight*240/384;c.right=halfHeight*240/384;c.top=halfHeight;c.bottom=-halfHeight;c.position.set(0,0,5);c.lookAt(0,0,0);c.updateProjectionMatrix();

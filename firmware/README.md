...
This is a buildable ESP-IDF/PlatformIO skeleton for the **Waveshare ESP32-C6 Touch AMOLED 1.64** module. It currently initializes only the standard ESP-IDF logger and emits a JSON heartbeat. It compiles the generated protocol-v1 version and limit constants, but does **not** parse protocol messages or initialize the AMOLED, touch controller, Wi-Fi, TLS, pairing, LVGL, or persistent settings; those are tracked by issue #5 and require validation against Waveshare's board documentation and BSP.
...
Expected serial messages contain `"state":"baseline"`, `"protocol_contract":"1.0"`, and the schema-derived maximum message size, followed by periodic `"state":"alive"` heartbeats. Flashing and physical behavior are not covered by CI.
